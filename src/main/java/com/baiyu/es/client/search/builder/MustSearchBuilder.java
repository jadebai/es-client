...
/**
 * @author baiyu
 * @description: MustSearchBuilder
 * @date: 2018/11/6
 */
public class MustSearchBuilder extends AbstractSearchBuilder {

    public MustSearchBuilder(SearchRequestBuilder searchRequestBuilder, BoolQueryBuilder boolQueryBuilder, SearchTypeParam searchTypeParam) {
    ...
    void setQuery(BoolQueryBuilder boolQueryBuilder,List<QueryBuilder> queryBuilderList) {
        ...
        for (QueryBuilder queryBuilder : queryBuilderList){
            boolQueryBuilder.must(scoreWarp(queryBuilder));
        }
    }
}
